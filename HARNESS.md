...
多条 system 消息对模型来说优先级相同，都是"开发者指令"。顺序只影响**注意力分布**，不影响权限等级。但模型对开头和结尾的内容记忆更强（Primacy & Recency Effect），所以最重要的规则应放在第一条或最后一条 system 消息。

#### 消息顺序 = 时间顺序

`messages[]` 数组的索引顺序即时间线。模型把靠后的消息理解为"更近发生的事"，后出现的信息会覆盖前面的同类信息。这也是 `buildContext()` 按 L2→L1→L0 顺序组装的原因——越新的信息越靠后，模型越重视。
